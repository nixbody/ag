...
SCENARIO("Children are aligned according to their parent h_box alignment setting", "[h_box]")
{
	ag::h_box box;
	...
	GIVEN("the box alignment is set to a certain value")
	{
		WHEN("children positions are calculated by the box")
		{
			THEN("the resulting coordinates correspond to the set alignment")
			{
				...
				REQUIRE(box.children().back().get().x() == 5.0f);
				REQUIRE(box.children().back().get().y() == 0.0f);

				box.align = ag::box::alignment::top_center;
				REQUIRE(box.children().front().get().x() == 5.0f);
				REQUIRE(box.children().front().get().y() == 0.0f);
				REQUIRE(box.children().back().get().x() == 10.0f);
				REQUIRE(box.children().back().get().y() == 0.0f);
				...
				REQUIRE(box.children().front().get().x() == 0.0f);
				REQUIRE(box.children().front().get().y() == 7.5f);
				REQUIRE(box.children().back().get().x() == 5.0f);
				REQUIRE(box.children().back().get().y() == 7.5f);
				...
				REQUIRE(box.children().front().get().x() == 0.0f);
				REQUIRE(box.children().front().get().y() == 15.0f);
				REQUIRE(box.children().back().get().x() == 5.0f);
